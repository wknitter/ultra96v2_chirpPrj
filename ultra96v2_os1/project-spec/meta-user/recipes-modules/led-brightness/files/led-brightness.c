/*  led-brightness.c - The simplest kernel module.
...
MODULE_AUTHOR
    ("Xilinx Inc.");
MODULE_DESCRIPTION
    ("led-brightness - loadable module template generated by petalinux-create -t modules");
...
static irqreturn_t led_brightness_irq(int irq, void *lp)
{
	printk("led-brightness interrupt\n");
	return IRQ_HANDLED;
}

static int led_brightness_probe(struct platform_device *pdev)
{
	struct resource *r_irq; // Interrupt resources 
	struct resource *r_mem; // IO mem resources 
	struct device *dev = &pdev->dev;
	struct led_brightness_local *lp = NULL;
	...
	lp = (struct led_brightness_local *) kmalloc(sizeof(struct led_brightness_local), GFP_KERNEL);
	if (!lp) {
		dev_err(dev, "Cound not allocate led-brightness device\n");
		return -ENOMEM;
	}
	...
	if (!lp->base_addr) {
		dev_err(dev, "led-brightness: Could not allocate iomem\n");
		rc = -EIO;
		goto error2;
	}
	...
	if (!r_irq) {
		...
		dev_info(dev, "led-brightness at 0x%08x mapped to 0x%08x\n",
			(unsigned int __force)lp->mem_start,
			(unsigned int __force)lp->base_addr);
		return 0;
	}
	lp->irq = r_irq->start;
	rc = request_irq(lp->irq, &led_brightness_irq, 0, DRIVER_NAME, lp);
	if (rc) {
	...
	dev_info(dev,"led-brightness at 0x%08x mapped to 0x%08x, irq=%d\n",
		(unsigned int __force)lp->mem_start,
		(unsigned int __force)lp->base_addr,
		lp->irq);
	return 0;
...
static int led_brightness_remove(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct led_brightness_local *lp = dev_get_drvdata(dev);
	free_irq(lp->irq, lp);
...
#ifdef CONFIG_OF
static struct of_device_id led_brightness_of_match[] = {
	{ .compatible = "vendor,led-brightness", },
	{ /* end of list */ },
};
MODULE_DEVICE_TABLE(of, led_brightness_of_match);
#else
# define led_brightness_of_match
#endif
...
static struct platform_driver led_brightness_driver = {
	...
	.probe		= led_brightness_probe,
	.remove		= led_brightness_remove,
};

static int __init led_brightness_init(void)
{
	printk("<1>Hello module world.\n");
	printk("<1>Module parameters were (0x%08x) and \"%s\"\n", myint,
	       mystr);

	return platform_driver_register(&led_brightness_driver);
}


static void __exit led_brightness_exit(void)
{
	platform_driver_unregister(&led_brightness_driver);
	printk(KERN_ALERT "Goodbye module world.\n");
}

module_init(led_brightness_init);
module_exit(led_brightness_exit);
